...
def getKeywordByURL(url, nKeywords=10):
...
def getKeywordData(keywords, url):
    ...
    try:
        ...
        for keyword_idea in keyword_ideas['KeywordIdea']:
            ...
            k['competition'] = keyword_idea.Competition
            m = keyword_idea.MonthlySearchCounts.long
            k['volume'] = m[-1]
            # print('volume = ',k['volume'],'  -> ',type(k['volume']))
            k['avgVolume'] = round(sum(m) / len(m))
            # print('avg_volume = ', k['avg_volume'], '  -> ', type(k['avg_volume']))
            k['relevance'] = keyword_idea.Relevance
            k['suggestedBid'] = keyword_idea.SuggestedBid
            k['avgCpc'] = 0.0
            k['avgPosition'] = 0.0
            k['clicks'] = 0.0
            k['ctr'] = 0.0
        ...
        for i in range(len(estimates)):
            keywords[i]['avgCpc'] = round((estimates[i].Maximum.AverageCpc + estimates[i].Minimum.AverageCpc) / 2, 2)
            keywords[i]['avgPosition'] = round(
                (estimates[i].Maximum.AveragePosition + estimates[i].Minimum.AveragePosition) / 2, 2)
            keywords[i]['clicks'] = round((estimates[i].Maximum.Clicks + estimates[i].Minimum.Clicks) / 2, 2)
            keywords[i]['ctr'] = round((estimates[i].Maximum.Ctr + estimates[i].Minimum.Ctr) / 2, 2)
            keywords[i]['impressions'] = round(
                (estimates[i].Maximum.Impressions + estimates[i].Minimum.Impressions) / 2, 2)
            keywords[i]['totalCost'] = round((estimates[i].Maximum.TotalCost + estimates[i].Minimum.TotalCost) / 2, 2)
